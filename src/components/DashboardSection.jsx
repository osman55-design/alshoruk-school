...
export default function DashboardSection({ onBack, currentUser }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', direction: 'rtl' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #e2e8f0', paddingBottom: '12px' }}>
        <div>
          <h3 style={{ margin: 0, color: '#047857', fontWeight: '900', fontSize: '18px' }}>⚙️ لوحة التحكم العامة للنظام</h3>
          <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '12px' }}>إدارة العمليات المدرسية والبيانات المتاحة</p>
        </div>
        <button onClick={onBack} style={{ background: '#0284c7', color: '#fff', border: 'none', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px' }}>↩️ عودة للرئيسية</button>
      </div>

      <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
        <h4 style={{ margin: '0 0 12px 0', color: '#047857', fontSize: '15px' }}>➕ إضافة موظف جديد وتحديد الصلاحيات</h4>
      ...
      <div>
        <h4 style={{ margin: '0 0 10px 0', color: '#0f172a', fontSize: '15px' }}>👥 قائمة الموظفين والصلاحيات ({users.length})</h4>
        {loading ? (
          <p style={{ textAlign: 'center', color: '#64748b' }}>جاري التحميل...</p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {users.map((u) => (
              <div key={u.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <h5 style={{ margin: '0 0 4px 0', fontSize: '14px', color: '#0f172a' }}>{u.full_name}</h5>
                  <span style={{ fontSize: '11px', color: '#64748b' }}>اسم الدخول: <b style={{ color: '#047857' }}>{u.username}</b> | الرتبة: <b style={{ color: '#2563eb' }}>{u.role}</b></span>
                </div>
                <button onClick={() => handleDeleteUser(u.id)} style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fee2e2', padding: '5px 10px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px' }}>🗑️ حذف</button>
              </div>
            ))}
          </div>
        )}

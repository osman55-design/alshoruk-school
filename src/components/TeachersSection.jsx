      ...
      {showQuerySection && (
        <div style={{ background: '#fff', padding: '25px', borderRadius: '12px', border: '1px solid #e5e7eb', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', marginBottom: '30px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '25px' }}>
            <button onClick={() => setActiveSubject(null)} style={{ padding: '8px 15px', borderRadius: '6px', border: '1px solid #ccc', cursor: 'pointer', backgroundColor: activeSubject === null ? '#1e3a8a' : '#fff', color: activeSubject === null ? '#fff' : '#333', fontWeight: 'bold' }}>
              الكل ({teachers.length})
            </button>
            {querySubjectsList.map((sub, idx) => (
              <button key={idx} onClick={() => setActiveSubject(sub)} style={{ padding: '8px 15px', borderRadius: '6px', border: '1px solid #ccc', cursor: 'pointer', backgroundColor: activeSubject === sub ? '#1e3a8a' : '#fff', color: activeSubject === sub ? '#fff' : '#333' }}>

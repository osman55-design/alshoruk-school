...
export default function App() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#ffffff', direction: 'rtl', fontFamily: 'Arial' }}>
      ...
      {/* الشريط العلوي الأخضر والذهبي المتكامل */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '15px', padding: '15px 5%', background: 'linear-gradient(90deg, #115e59 0%, #14b8a6 100%)', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', borderBottom: '4px solid #d4af37' }}>
        ...
        {!isLoggedIn ? (
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center' }}>
            ...
            <button style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontWeight: 'bold', backgroundColor: activeTab === 'classes' ? '#d4af37' : '#fff', color: activeTab === 'classes' ? '#fff' : '#115e59' }} onClick={() => setActiveTab('classes')}>الفصول</button>
            <button style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontWeight: 'bold', backgroundColor: activeTab === 'teachers' ? '#d4af37' : '#fff', color: activeTab === 'teachers' ? '#fff' : '#115e59' }} onClick={() => setActiveTab('teachers')}>المعلمين</button>
            <button style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontWeight: 'bold', backgroundColor: activeTab === 'accounts' ? '#d4af37' : '#fff', color: activeTab === 'accounts' ? '#fff' : '#115e59' }} onClick={() => setActiveTab('accounts')}>الحسابات</button>
            <button style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontWeight: 'bold', backgroundColor: activeTab === 'results' ? '#d4af37' : '#fff', color: activeTab === 'results' ? '#fff' : '#115e59' }} onClick={() => setActiveTab('results')}>النتيجة</button>
      ...
      <div style={{ padding: '40px 5%', flex: '1', backgroundColor: '#ffffff' }}>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '40px' }}>
              ...
              <div style={{ width: '100%', background: 'rgba(255, 255, 255, 0.05)', padding: '30px 20px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px', width: '100%', direction: 'rtl' }}>
                  ...
                  <div style={{ background: '#ffffff', borderRadius: '15px', padding: '15px', textAlign: 'center', border: '1px solid #e2e8f0', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
                    ...
                    <p style={{ margin: '0', color: '#334155', fontWeight: 'bold', fontSize: '15px' }}>الاستاذ كمال الدين مجذوب الطيب</p>
                  </div>
                  {/* البطاقة 2 */}
                  <div style={{ background: '#ffffff', borderRadius: '15px', padding: '15px', textAlign: 'center', border: '1px solid #e2e8f0', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
                  ...
                  <div style={{ background: '#ffffff', borderRadius: '15px', padding: '15px', textAlign: 'center', border: '1px solid #e2e8f0', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
                    ...
                    <p style={{ margin: '0', color: '#334155', fontWeight: 'bold', fontSize: '15px' }}>الاستاذ محمد كمال الدين مجذوب</p>
                  </div>
                  ...
                  <div style={{ background: '#ffffff', borderRadius: '15px', padding: '15px', textAlign: 'center', border: '1px solid #e2e8f0', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
                    ...
                    <p style={{ margin: '0', color: '#334155', fontWeight: 'bold', fontSize: '15px' }}>الاستاذة لينا كمال الدين مجذوب</p>
                  </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(290px, 1fr))', gap: '30px' }}>
              <div style={{ background: '#fff', padding: '25px', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.04)', borderTop: '5px solid #115e59', borderLeft: '1px solid #e2e8f0', borderRight: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0' }}>
                ...
                <p style={{ color: '#475569', lineHeight: '1.7', fontSize: '15px', margin: 0 }}>مدرسة الشروق السودانية المتكاملة هي صرح تعليمي رائد مخصص لتقديم المنهج السوداني الرصين بكفاءة وجودة عالية.</p>
              </div>
              ...
              <div style={{ background: '#fff', padding: '25px', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.04)', borderTop: '5px solid #d4af37', borderLeft: '1px solid #e2e8f0', borderRight: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0' }}>
                ...
                <p style={{ color: '#475569', lineHeight: '1.7', fontSize: '15px', margin: 0 }}>تتضمن هذه البوابة الإلكترونية المتقدمة لوحة تحكم ونظاماً برمجياً لإدارة شؤون المدرسة بأعلى كفاءة وسرعة.</p>
              </div>
            </div>
        ...
        {/* عرض القوائم الستة المتكاملة ديناميكياً بناءً على الزر النشط */}
        {isLoggedIn && (
          <div className="content-fade-in">
            ...
            {activeTab === 'dashboard' && <DashboardSection users={usersList} setUsers={setUsersList} onBack={() => setActiveTab('dashboard')} />}
          </div>
        )}
      </div>

      {/* نافذة تسجيل الدخول بالأخضر والذهبي */}
      {showLoginModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(19,78,74,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 3000 }}>
          <form onSubmit={handleLogin} style={{ background: '#fff', padding: '30px', borderRadius: '16px', width: '320px', position: 'relative', borderTop: '6px solid #d4af37', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
            ...
            <button type="submit" disabled={loading} style={{ width: '100%', padding: '12px', background: 'linear-gradient(90deg, #115e59 0%, #14b8a6 100%)', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '15px' }}>

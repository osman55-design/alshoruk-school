  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', direction: 'rtl' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #e2e8f0', paddingBottom: '12px' }}>
        <div>
          <h3 style={{ margin: 0, color: '#047857', fontWeight: '900', fontSize: '18px' }}>⚙️ نظام إدارة المستخدمين والصلاحيات</h3>
          <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '12px' }}>إدارة الحسابات وتوزيع الأدوار والمراحل الدراسية</p>
        </div>
        {onBack && (
          <button onClick={onBack} style={{ background: '#0284c7', color: '#fff', border: 'none', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px' }}>
            ↩️ العودة
          </button>
        )}
      </div>
      ...
      <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
        <h4 style={{ margin: '0 0 12px 0', color: '#047857', fontSize: '15px' }}>➕ إضافة مستخدم جديد</h4>
        <form onSubmit={handleAddUser} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div>
            <label style={{ ...labelStyle, marginBottom: '8px', display: 'block', color: '#0284c7' }}>🏫 المراحل الدراسية المصرحة:</label>
      ...
      <div>
        <h4 style={{ margin: '0 0 10px 0', color: '#0f172a', fontSize: '15px' }}>👥 قائمة المستخدمين المسجلين ({users.length})</h4>
        {loading ? (
